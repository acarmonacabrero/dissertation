...
empirical_RSsobolSample = function(data=data_gsa[, -1], n, order=2, type=sobolEff){
  # Type can be sobol, sobol2007 or sobolEff
  X1 = data.frame(matrix(nrow=n, ncol=dim(data)[2]))
  X2 = data.frame(matrix(nrow=n, ncol=dim(data)[2]))
  var = 0
...
if (args[4] == 'LHS'){
  n = args[5]  # LHS
  xx = LHSsobolSample(X, order=2, strtoi(args[5]), type=sobolEff)
}

if (args[4] == 'RS'){
  n = args[5]  # RS
  xx = RSsobolSample(X, order=2, strtoi(args[5]), type=sobolEff)
}

if (args[4] == 'eRS'){
  n = args[5]  # eRS
  xx = empirical_RSsobolSample(X, order=2, strtoi(args[5]), type=sobolEff)
}
