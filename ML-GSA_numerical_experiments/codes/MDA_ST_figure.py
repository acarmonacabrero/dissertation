...
plot_list = [xgb_add, rf_add, xgb_add, rf_add]
fig, axes = plt.subplots(1, 4, sharex=True, sharey=True, figsize=[14, 7])
for plot_index, plot_item in enumerate(plot_list):
    for experiment in rf_add['S'].keys():
        experiment_label = r'$N_T = ' + experiment.split(sep='_')[0] + ', R_\epsilon = ' + \
        ...
        for variable in range(k):
            ...
            for realization in range(n_rea):
                if (plot_index == 0) or (plot_index == 1):
                    ...
                    realization_list.append(plot_item['Perm_imp'][experiment][realization]['importances_mean'][variable]/
                                            plot_item['Perm_imp'][experiment][realization]['importances_mean'].sum())
            # print(variable, np.mean(realization_list), np.std(realization_list))
        # print(plot_index, analytical_sti)
        ...
        axes[plot_index].errorbar(x=df_imp['mean'], xerr=df_imp['ci'], y=df_imp['Analytical ST'], fmt='o', capsize=5,
                                  markeredgewidth=1, label=experiment_label)
        if funct_name == 'ishigami':
            axes[plot_index].plot([-0.05, .8], [-.05, .8], 'gray', linewidth=0.5)
            max_xlim = .8
        else:
            axes[plot_index].plot([-0.05, .4], [-.05, .4], 'gray', linewidth=0.5)
            max_xlim = .4
    # plt.plot([0, 1], [0, 1], color='gray')
    # plt.xlim([-0.05, 1.05])
    # plt.ylim([-0.05, 1.05])
# axes[3].legend(loc=0)
axes[1].legend(loc='upper center', bbox_to_anchor=(1, -0.1), fancybox=True, ncol=5)
# plt.xlim([-0.07, max_xlim + 0.05])
axes[0].set_ylabel(r'Analytical $S_T$')
axes[0].set_xlabel(r'$S_T$ XGB')
axes[1].set_xlabel(r'$S_T$ RF')
axes[2].set_xlabel('MDA XGB')
axes[3].set_xlabel('MDA RF')
# plt.tight_layout()
fig.subplots_adjust(bottom=0.2)
plt.savefig('final_figures/good_quality/new_MDA_ST_' + funct_name + '_' + error_type + '.png', dpi=500)
